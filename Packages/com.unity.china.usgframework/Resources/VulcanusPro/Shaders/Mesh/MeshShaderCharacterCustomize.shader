//
Shader "MeshShaderCharacterCustomize"
{
	Properties {
	...
	SubShader {
		...
		Pass
		{
			...
			half4 frag(v2f i) : SV_Target
			{
				...
				{
					Light light = GetAdditionalLight(lightIndex, i.posWS, i.shadowCoord);
					half3 attenuatedLightColor = light.color * (light.distanceAttenuation * light.shadowAttenuation);
					color.xyz += LightingLambert(attenuatedLightColor, light.direction, i.normal);
					emission += LightingSpecular(attenuatedLightColor, light.direction, i.normal, i.viewDir, color, color.a);
				}
