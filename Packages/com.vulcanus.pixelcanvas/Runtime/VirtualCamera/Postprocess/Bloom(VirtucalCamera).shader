Shader "VirtualCamera//Bloom"
{
    HLSLINCLUDE
        ...
        half4 FragPrefilter(v2f input) : SV_Target
        {
            ...
            color.a = 0;
            half3 mask = step(1.000001, color.rgb);
            if (0 < mask.r + mask.g + mask.b)
                color.a = 1;
                //color.a = (color.x + color.y + color.z) * 0.133;
        ...
        half4 Upsample(float2 uv)
        {
            ...
            half4 result = lerp(highMip, lowMip, Scatter);
//            result.a = saturate(result.a);
            return result;
        }

        half4 FragUpsample(v2f input) : SV_Target
        {
            UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(input);
