...
namespace UnityEngine.AddressableAssets
{
    internal class AddressablesImpl : IEqualityComparer<IResourceLocation>
    {
        ...
        // Added: Remove specific primary key from all ResourceLocationMap locators
        public bool RemoveResourceLocationByPrimaryKey(object primaryKey)
        {
            if (primaryKey == null)
                return false;
            ...
            {
                var rlm = locatorInfo.Locator as ResourceLocationMap;
                if (rlm == null)
                    continue;

                // If the primary key exists in this locator, remove its entry
                if (rlm.Locations != null && rlm.Locations.Remove(primaryKey))
                    removedAny = true;
            }
            return removedAny;
        }
