...
namespace UnityEditor.AddressableAssets.Build
{
    ...
    public class AddressablesBuildMenuUpdateVulcanus : AddressableAssetsSettingsGroupEditor.IAddressablesBuildMenu
    {
        /// <inheritdoc />
        public virtual string BuildMenuPath
        {
            get => "Build Vulcanus (Only GameFramework)";
        }
        ...
        bool OnUpdateBuild(AddressablesDataBuilderInput input)
        {
            ...
            if (!string.IsNullOrEmpty(path) && doContentUpdate)
            {
                if (!File.Exists(path))
                {
                    ...
                    //if (selectBin)
                    path = null; //ContentUpdateScript.GetContentStateDataPath(false);
                    // else
                    // {
                    //     Debug.Log("Update a Previous Build cancelled by user.");
                    //     buildCancelled = true;
