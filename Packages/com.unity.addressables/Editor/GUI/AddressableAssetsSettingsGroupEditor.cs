...
{
    [Serializable]
    public class AddressableAssetsSettingsGroupEditor
    {
        ...
        public struct BuildMenuContext
        {
            ...
            GUILayout.BeginHorizontal(EditorStyles.toolbar);
            {
                ...
                if (EditorGUI.DropdownButton(rBuild, guiBuild, FocusType.Passive, EditorStyles.toolbarDropDown))
                {
                    ...
                    foreach (IAddressablesBuildMenu buildOption in displayMenus)
                    {
                        ...
                        {
                            BuildMenuContext context = new BuildMenuContext()
                            { buildScriptIndex = -1, BuildMenu = buildOption, Settings = settings, BuildTarget = EditorUserBuildSettings.activeBuildTarget };
        ...
        public static void OnBuildAddressables(object ctx)
        {
        ...
        public static void OnBuildAddressables(BuildMenuContext context)
        {
            ...
            var builderInput = new AddressablesDataBuilderInput(context.Settings, context.BuildTarget);
        ...
        public static void OnBuildAddressablesWithBuildResult(BuildMenuContext context, out AddressablesPlayerBuildResult rst)
        {
            rst = null;
            ...
            {
                Addressables.LogError("Addressable content build failure : null build menu context");
                rst = new AddressablesPlayerBuildResult(){ Error = "Null build menu context" };
                return;
            }
            ...
            if (!HandlePreBuild(context, builderInput))
            {
                rst = new AddressablesPlayerBuildResult(){ Error = "PreBuild Fail" };
                return;
            }

            AddressableAssetSettings.BuildPlayerContent(out rst, builderInput);
        ...
        public static bool HandlePreBuild(BuildMenuContext context, AddressablesDataBuilderInput builderInput)
        {
