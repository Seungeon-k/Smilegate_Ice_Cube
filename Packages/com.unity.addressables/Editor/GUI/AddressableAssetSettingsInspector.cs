...
namespace UnityEditor.AddressableAssets.GUI
{
    ...
    class AddressableAssetSettingsInspector : Editor
    {
        ...
        static FoldoutSessionStateValue BuilderFoldout = new FoldoutSessionStateValue("Addressables.BuilderFoldout");
        GUIContent m_BuilderHeader;
        ...
        void OnEnable()
        {
            ...
            m_BuilderHeader = new GUIContent("Builder");
        ...
        public override void OnInspectorGUI()
        {
            ...
            GUILayout.Space(12);
            BuilderFoldout.IsActive = AddressablesGUIUtility.BeginFoldoutHeaderGroupWithHelp(BuilderFoldout.IsActive, m_BuilderHeader);
            if (BuilderFoldout.IsActive)
            {
                EditorGUILayout.BeginHorizontal();
                {
                    ...
                    var newTargetFolderObject = EditorGUI.ObjectField(fieldRect, "Target Folder", m_AasTarget.targetFolderObject, typeof(UnityEngine.Object), false);
                    if (ccs.changed)
                    {
                        m_QueuedChanges.Add(() =>
                        {
                            m_AasTarget.targetFolderObject = newTargetFolderObject;
                            if (m_AasTarget.targetFolderObject == null) return;

                            m_AasTarget.groups.Clear();

                            var folderPath = AssetDatabase.GetAssetPath(m_AasTarget.targetFolderObject);
                            m_AasTarget.groups = AssetDatabase.FindAssets($"t:{nameof(AddressableAssetGroup)}", new []{ folderPath }).Select(x =>
                                {
                                    var assetPath = AssetDatabase.GUIDToAssetPath(x);
                                    return AssetDatabase.LoadAssetAtPath<AddressableAssetGroup>(assetPath);
                                })
                                .ToList();
                        });
                    }
                ...
                VAddressableAssetBuilder.DoGUI(m_AasTarget.Builder, serializedObject.targetObject);
