...
function this.OnStart()    
    ...
    local bombObject = script.parent:Find('Equip/Thrown Bomb/Script')
    bombScript = bombObject:GetLua()
...
function this.OnAttackBegin(item)
    ...
    local thScriptObject = th:Find('Script')
    local thScript = thScriptObject:GetLua()
    if thScript then
        thScript.ContinueWithTime(bombScript.GetLeftTime())
    end
...
__EX_FUNCTION__(this)
function this.BombExplodedOnEvent()

    checkVisible = true
    if this.item then
        this.item:NotifyItemAutoUseCompleted()
    end
