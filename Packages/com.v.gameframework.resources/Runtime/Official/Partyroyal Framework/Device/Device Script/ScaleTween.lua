...
this.ScaleDelta = __EX_VARIABLE__.vector3()
...
local initStartScale = nil
local initTargetScale = nil
local addType = true
...
local function ApplyTween(t)
    ...
    local scale = initStartScale + (initTargetScale - initStartScale) * lerpT
...
function this.UpdateLoopFinish()
    ...
    if this.PingPong then
        ...
        isStarted = false
        if addType then
            initStartScale = targetTransform.localScale
            initTargetScale = initStartScale + this.ScaleDelta
        end
