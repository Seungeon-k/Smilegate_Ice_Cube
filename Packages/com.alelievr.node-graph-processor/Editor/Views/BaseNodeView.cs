	...
	public class BaseNodeView : NodeView
	{
		...
		protected virtual void DrawDefaultInspector(bool fromInspector = false)
		{
			...
			foreach (var field in fields)
			{
				...
				var elem = AddControlField(field, displayName, showInputDrawer, valueChangedCallBack);
        ...
        protected Action valueChangedCallBack = null;
		...
		internal void SyncSerializedPropertyPathes()
		{
			...
			foreach (var propertyField in this.Query<PropertyField>().ToList())
			{
				...
				propertyField.bindingPath = s_ReplaceNodeIndexPropertyPath.Replace(propertyField.bindingPath, m =>
                {
                    return m.Groups[1].Value + nodeIndexString + m.Groups[3].Value;
                });
