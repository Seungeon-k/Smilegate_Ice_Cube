...
namespace GraphProcessor
{
	...
	public class BaseGraphView : GraphView, IDisposable
	{
		...
		protected Dictionary<Type, (Type nodeType, MethodInfo initalizeNodeFromObject)> nodeTypePerCreateAssetType = new Dictionary<Type, (Type, MethodInfo)>();
		protected Dictionary<Type, (Type nodeType, MethodInfo initalizeNodeFromObject)> nodeTypePerCreateCompType = new Dictionary<Type, (Type, MethodInfo)>();
		...
		protected virtual void DragPerformedCallback(DragPerformEvent e)
		{
			...
			if (DragAndDrop.objectReferences.Length > 0)
			{
				...
				foreach (var obj in DragAndDrop.objectReferences)
				{
					...
					foreach (var kp in nodeTypePerCreateCompType)
					{
						if (objectType.IsAssignableFrom(typeof(UnityEngine.GameObject)) &&
						    (obj as GameObject)?.GetComponent(kp.Key) != null)
						{
							try
							{
								var node = BaseNode.CreateFromType(kp.Value.nodeType, mousePos);
								var gameObj = obj as GameObject;
								var component = gameObj?.GetComponent(kp.Key);
								if ((bool)kp.Value.initalizeNodeFromObject.Invoke(node, new []{component}))
								{
									AddNode(node);
									createdFrom = true;
									break;
			...
			foreach (var nodeInfo in NodeProvider.GetNodeMenuEntries(graph))
			{
				...
				foreach (var i in interfaces)
				{
					if (i.IsGenericType && i.GetGenericTypeDefinition() == typeof(ICreateNodeFrom<>))
					{
						...
						if (typeof(UnityEngine.Component).IsAssignableFrom(genericArgumentType))
						{
							if (initializeFunction.DeclaringType == nodeInfo.type)
								nodeTypePerCreateCompType[genericArgumentType] = (nodeInfo.type, initializeFunction);
						}
						else
						{
        ...
        public virtual GroupView AddGroup(Group block)
        {
		...
		public virtual GroupView AddGroupView(Group block)
		{
			var c = new GroupView();
...
#if UNITY_2020_1_OR_NEWER
        public virtual StickyNoteView AddStickyNote(StickyNote note)
        {
